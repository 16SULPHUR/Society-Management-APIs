...
async function convert(options, userSettings) {
    const isDocument = options.type === exports.ConversionType.DOCUMENT;
    const body = {
        paths: options.paths,
...
const createSignature = (...args) => {
    return Promise.resolve().then(function () { return require('./createSignature.node.389d4e7c.cjs'); }).then((m) => m.createSignature(...args));
};
...
class UploadcareAuthSchema {
    ...
    constructor(options) {
        ...
        if (md5Loader) {
            ...
            this._md5Loader = Promise.resolve().then(function () { return require('./md5.node.bb9d9377.cjs'); }).then((m) => m.md5);
        }
    }
    async md5(input) {
